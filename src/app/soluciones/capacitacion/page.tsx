...
const service = services.find((s) => s.id === 'capacitacion')!;
...
const gradientSection = 'section-gradient-emerald';
const orbColor = 'orb orb-emerald';
const accentText = 'text-accent-emerald';
const accentBg = 'bg-accent-emerald';
            ...
            {service.features.map((feature, i) => (
              <RevealWrapper key={i} delay={i < 4 ? i + 1 : undefined}>
                <GlassCard hover className="p-6 h-full">
                  <div className={`w-10 h-10 rounded-xl bg-accent-emerald/10 flex items-center justify-center mb-4 ${accentText}`}>{icons.GraduationCap}</div>
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6">
            {service.useCases.map((useCase, i) => (
              <RevealWrapper key={i} delay={i < 3 ? i + 1 : undefined}>
                <GlassCard hover className="p-8 h-full">
                  <div className={`w-10 h-10 rounded-full bg-accent-emerald/10 flex items-center justify-center mb-4 ${accentText}`}>{icons.Target}</div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <RevealWrapper delay={1}>
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-txt-inv text-center leading-[1.1]">
              Transforma a tu equipo en expertos en IA
            </h2>
          </RevealWrapper>
          <RevealWrapper delay={2}>
            <p className="mt-6 text-lg text-txt-inv-2 text-center max-w-2xl mx-auto">
              Descubre nuestros programas de capacitación diseñados para cada rol de tu organización.
            </p>
          </RevealWrapper>
          <RevealWrapper delay={3}>
